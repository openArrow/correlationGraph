...
for(r1 in c(1:(row))) 
{
  ...
  { 
    ...
    for(r2 in c(1:row)) 
    {
      ...
      {
        cl<-cl+1
        if(!are.connected(g,rl,cl) && dataMatrix[r1,c1,1] != 168 && dataMatrix[r2,c2,1] != 168 )
        {
          d<-cor(dataMatrix[r1,c1,1:height],dataMatrix[r2,c2,1:height],method = "pearson")
...
cc<-sum(transitivity(g, type=c("localundirected"),isolate="zero"))/totLen
hist(degNew[degNew<35],main="Histogram for Degree Distribution", xlab="Degree",ylab = "no.Of Vertices",xlim=c(1,30),las=1,breaks=20)
